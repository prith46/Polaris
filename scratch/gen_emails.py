...
# Generate 45 non-task emails
non_task_emails_pool = [
    {
...
# Ensure we have exactly 50 task emails and 45 non-task emails
assert len(task_emails_pool) == 50
assert len(non_task_emails_pool) == 45

# Distribute 15 unread statuses among task_emails_pool
# The other 35 task emails will be unread=False
# Non-task emails are all unread=False
random.seed(42)  # For reproducible shuffling
unread_indices = set(random.sample(range(50), 15))

task_emails = []
for idx, item in enumerate(task_emails_pool):
    task_emails.append({
        "from": item["from"],
        ...
        "unread": idx in unread_indices,
        "starred": random.choice([True, False, False, False]),
        "important": random.choice([True, False, False, False])
    })

non_task_emails = []
for item in non_task_emails_pool:
    non_task_emails.append({
        "from": item["from"],
        "subject": item["subject"],
        "body": item["body"],
        "avatarLetter": item["avatarLetter"],
...
# Combine and shuffle emails 6-100 (which is 95 emails: 50 tasks + 45 non-tasks)
combined_pool = task_emails + non_task_emails
random.shuffle(combined_pool)

# Verify counts
assert len(combined_pool) == 95

# Generate timestamps spread across past 2 weeks
# E.g. "June 24", "June 23", "June 20", "June 15", "June 12"
days_ago_labels = [
    "June 25", "June 24", "June 23", "June 22", "June 21",
    "June 20", "June 19", "June 18", "June 17", "June 16",
    "June 15", "June 14", "June 13", "June 12", "June 11"
]

all_emails = list(top_5)
for i, email in enumerate(combined_pool):
    email["id"] = f"email-{i+6}"
    # Assign timestamp
    day_label = days_ago_labels[i % len(days_ago_labels)]
    hour = 9 + (i % 12)
    minute = 10 + (i % 50)
    email["time"] = f"{day_label} {hour}:{minute:02d} AM"
    all_emails.append(email)

assert len(all_emails) == 100

# Write to js-style output file
with open("e:/polaris/scratch/emails_array.json", "w", encoding="utf-8") as f:
    json.dump(all_emails, f, indent=2, ensure_ascii=False)

print("Generated exactly 100 emails.")
